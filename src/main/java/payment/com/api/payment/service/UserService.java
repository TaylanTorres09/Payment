...
public class UserService {
     ...
     * @return
     */
    public ResponseEntity<?> loginUser(LoginUser form) {
        Iterable<User> users = ur.findAll();
        for(User u: users) {
            if ((u.getEmail().equals(form.getEmail())) && (u.getPassword().equals(form.getPassword()))) {
                rm.setMensage("Login feito com sucesso");
                return new ResponseEntity<ResponseModel>(rm, HttpStatus.OK);
            }
        }
        rm.setMensage("Usuário ou Senha Incorretos");
        return new ResponseEntity<ResponseModel>(rm, HttpStatus.BAD_REQUEST);
    }
}